...
func TestPreemption(t *testing.T) {
	applicationTest(t, true, testutil.SpinLoop, func(c *vCPU, regs *syscall.PtraceRegs, pt *pagetables.PageTables) bool {
		...
		for i := 0; i < 1000; i++ {
			...
			// Should still be in guest mode, with vCPU locked here.
			if got := atomic.LoadUint32(&c.state); got != vCPUGuest|vCPUUser|vCPUWaiter {
				t.Errorf("vCPU not in vCPUGuest|vCPUUser|vCPUWaiter state: got %v", got)
			}
			c.unlock()
